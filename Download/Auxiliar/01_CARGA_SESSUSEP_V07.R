...
# load ----------------------------------------------------------------------------------------------------------------------
  ...
  if(x_md5sum_new!=x_md5sum_old){
    ...
    for( i in tb_files$ID){
      ...
      xi = tb_files$ARQUIVO[i]
      cat(rep('-',100),'\n',format(Sys.time(),'%H:%M:%S|'),'Carregando: ',xi,' [',i,']',sep='')
      ...
      tb_modelo = 
        ...
        arrange(ID) %>% 
        # select(CAMPO, TIPO) %>% {
        #   read.table(text='',col.names=.[]$CAMPO %>% replace_non_ascii %>% str_to_upper,colClasses=.[]$TIPO)
        # } %>% 
        as.data.frame
      # glimpse(tb_modelo)
      ...
      suppressWarnings({
        try({ 
          for(xj in names(tb_sample)){
            class(tb_sample[, xj]) = tb_modelo %>% filter(CAMPO==xj) %>% select(TIPO) %>% unlist %>% unname
          }
        })
      })
      ...
      paste0('SELECT TOP 30 * FROM ',x_name_table_dest) %>% dbGetQuery(w_con,.)
      
      if ( dbListTables(w_con,table_name = x_name_table_dest) %>% length > 0 ){
        cat(format(Sys.time(),'%H:%M:%S|'),'Finalizado: ',xi,' carregado em ',x_name_table_dest,'\n\n',sep='')
      }
