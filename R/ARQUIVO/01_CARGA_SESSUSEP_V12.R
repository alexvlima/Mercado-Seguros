...
# load ----------------------------------------------------------------------------------------------------------------------
      ...
      for( i in tb_files$ID){
        ...
        suppressWarnings({
          try({ 
            ...
            paste0("DROP TABLE IF EXISTS ",sub(x_table_prefix,'',x_name_temp)) %>% dbSendQuery(w_con,.)
          })
        })
        ...
        suppressWarnings({
          x_query = 
            ...
            paste0('SELECT \n',.,'\nINTO ',x_name_dest,'\nFROM ',x_name_temp,'\n') %>% 
            paste0(ifelse(sub(x_table_prefix,'',x_name_temp)=='SES_DIVERSOS',"WHERE DESCRICAO != 'descricao'",''))
        })
        # cat(x_query)
        ...
        if ( dbListTables(w_con,table_name = x_name_temp) %>% length > 0 & n_rows > 0){
          cat(
  ...
  suppressWarnings({
    try({   
      ...
      dbWriteTable(w_con, x_name_versao, tb_md5sum, overwrite = T, append = F, row.names = F)
    })
  })  
