...
# init -------------------------------------------------------------------------------------------------------
  ...
  if(Sys.getenv('https_proxy') == ''){
    Sys.setenv(
      https_proxy = x_proxy,
      https_proxy_user = paste(x_user, .rs.askForPassword("https: Senha"), sep =':')
    )
  }
...
# param ------------------------------------------------------------------------------------------------------
  ...
  w_con = 
    ...
    dbConnect(odbc(),.connection_string = ., encoding = "UTF-8", timeout = 10)
...
# scrap ------------------------------------------------------------------------------------------------------
  ...
  x_pos = 'Não foi possível acessar o site (verificar conexão ou proxy)'
  ...
  system.time({
    if(x_md5sum_new!=x_md5sum_old){
      ...
      for( i in tb_files$ID){
        ...
        # i=7 # para tesar a carga de um arquivo
        ...
        suppressWarnings({
          try({
            n_rows = 0
            n_rows = paste0('SELECT COUNT(*) FROM ',x_name_dest) %>% dbGetQuery(w_con,.) %>% unlist %>% unname
          })
        })
        
        x_check_table = dbListTables(w_con,table_name = x_name_dest) %>% length > 0 & n_rows > 0
        if ( x_check_table == T){
          cat(
        ...
        suppressWarnings({
          try({ 
            tb_controle = 
              ...
              filter(ARQUIVO == xi) %>% 
              mutate(
                MD5SUM_ARQUIVO_ZIP    = ifelse(x_check_table, x_md5sum_new, NA),
                DESCRICAO_SUSEP = ifelse(x_check_table, x_descr, NA),
                name = ARQUIVO %>% str_replace('.csv','') %>% str_to_upper %>% ifelse(x_check_table, ., NA),
              ) %>% 
              left_join(dbGetQuery(w_con,'SELECT * FROM SYS.TABLES')) %>% 
              select(ID, ARQUIVO, TAMANHO_KB, NOME_TABELA = name, DATA_CRIACAO = create_date, DESCRICAO_SUSEP, MD5SUM_ARQUIVO_ZIP) %>% 
              # filter(TABLE_NAME == x_name_dest) %>% 
              as.data.frame
            paste0("DELETE FROM ",x_name_carga," WHERE ARQUIVO = '",xi,"'") %>% dbSendQuery(w_con,.)
      ...
      w_email = COMCreate("Outlook.Application")$CreateItem(0)
      w_email[["to"]] = "alisson.santos@cnpbrasil.com.br"
      w_email[["subject"]] = "Carga SUSEP_SES"
      w_email[["htmlbody"]] = "<html> Carga SUSEP_SES Finalizada </html>"
      w_email$Send()
